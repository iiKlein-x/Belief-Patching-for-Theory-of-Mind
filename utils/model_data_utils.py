    ...
    def __init__(
        ...
        if model is None:
            ...
            model = AutoModelForCausalLM.from_pretrained(
                ...
                attn_implementation="eager" if "gpt2" not in model_name and "qwen3" not in model_name.lower() else None,
                device_map = 'auto',
        ...
        self.is_chat = 'chat' in m_name or 'instruct' in m_name.lower()
    def __repr__(self):
        return (
...
def load_hf_ds(ds_name,seed=0,tokenizer=None):
    ...
    if ds_name == 'csqa':
        ...
        with open('data/dataset_2nd.jsonl', 'r') as f:
            ds = [json.loads(l) for l in f]
        #return ds
    elif ds_name == 'tomi_1st':
            with open('data/dataset_2nd.jsonl', 'r') as f:
                ds = [json.loads(l) for l in f]
            #return ds
    elif ds_name == 'tomi_2nd':
                with open('data/dataset_2nd.jsonl', 'r') as f:
                    ds = [json.loads(l) for l in f]
                #return ds
    elif ds_name == 'hitom_1st':
                with open('data/dataset_2nd.jsonl', 'r') as f:
                    ds = [json.loads(l) for l in f]
                #return ds
    elif ds_name == 'hitom_2nd':
                    with open('data/dataset_2nd.jsonl', 'r') as f:
                        ds = [json.loads(l) for l in f]
                    #return ds
