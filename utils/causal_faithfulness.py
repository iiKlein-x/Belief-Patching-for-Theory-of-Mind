...
def calculate_STR_causal(
    ...
    scores = {},
    window = 1,
    layer_range=None,   # added layer parameter
):
    """
    ...
    if layer_range is None:
        layer_range = range(mt.num_layers)  # default to all layers
    ...
    original_range = find_token_range(mt.tokenizer, prompt[0], subject[0],include_chat_template = mt.is_chat,find_sub_range = not mt.is_chat,model_name=mt.model_name) # list of list, outer = number of words, inner = token pos
    cf_range = find_token_range(mt.tokenizer, prompt[1], subject[1],include_chat_template = mt.is_chat,find_sub_range = not mt.is_chat,model_name=mt.model_name)
    ...
    for tok_range_n_prompt in [(original_range,prompt[0]),(cf_range,prompt[1])]:
        ...
        if isinstance(input_until,str):
            end_pos = find_token_range(mt.tokenizer, p_prompt, input_until,include_chat_template = mt.is_chat,find_sub_range = not mt.is_chat,model_name=mt.model_name)[0]
            #print(f"input_until: '{input_until}'")
            #print(f"end_pos found: {end_pos}")
            #print(f"total prompt length: {len(p_prompt)}")
        else:
    ...
    patched_prob,patched_logit = trace_important_states_STR( # input,mask and token range are size of 2.
        mt.model,
        #mt.num_layers,
        layer_range,        # passing layer_range instead of mt.num_layers
        patch_input,
        answer_t,
...
def compute_causal_values_STR(ds,mt,args):
    ...
    # added this block to build the layer range from args
    layer_end = args.layer_end if args.layer_end != -1 else mt.num_layers
    layer_range = range(args.layer_start, layer_end)  # ← e.g. range(8, 16)
        ...
        for gen_type in ['answer','expl']:
            ...
            try:
                result = calculate_STR_causal(
                            ...
                            window = args.window,
                            layer_range=layer_range,  # pass it here
                        )
                ...
                continue
            numpy_result = {
                k: v.detach().float().cpu().numpy() if torch.is_tensor(v) else v
