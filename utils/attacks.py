...
def compute_causal_values(ds,mt,args):
    ...
    for sample in tqdm(ds,total = len(ds),desc = f'Getting GN attributions for {args.dataset_name}, {args.model_name}'):
        ...
        input_until = "\n\nAnswer: "
        subject_range_tokens = find_token_range(mt.tokenizer,sample['input_ids'],subject,include_chat_template = mt.is_chat,find_sub_range=not mt.is_chat,model_name=mt.model_name)
